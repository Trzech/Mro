...
void IntegralImageBuilder::buildForImage(unsigned char** inputBuffer, unsigned int** outputBuffer, int rows, int cols){
	int sum = 0;
	for (int j = 0; j < cols; ++j) {
		sum += inputBuffer[0][j];
		outputBuffer[0][j] =  sum;
...
void IntegralImageBuilder::buildForImageWithSquares(unsigned char** inputBuffer,
		...
		unsigned int** outputSquareBuffer, int rows, int cols){
	int sum = 0;
	int sumSquare = 0;
	for (int j = 0; j < cols; ++j) {
		sum += inputBuffer[0][j];
...
double IntegralImageBuilder::mean(unsigned int** integral, int i, int j, int k){
	//uwaga! nie uzględniono wartości brzegowych.
	unsigned long long int a = integral[i-k-1][j-k-1];
	unsigned long long int b = integral[i-k-1][j+k];
	unsigned long long int d = integral[i+k][j-k-1];
